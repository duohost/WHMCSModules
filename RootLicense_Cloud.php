 ...
 *
 * */
function RootLicense_Cloud_ConfigOptions()
{
    $configarray = array(
        // the dropdown field type renders a select menu of options
        "configoption" => array(
            "FriendlyName" => "CloudLinux type",
            'Type' => 'dropdown',
            'Options' => "Cloudlinux for cPanel,Cloudlinux",
            'description' => 'If you have cPanel on your server, select CloudLinux for cPanel.'
        ),
    );
    return $configarray;
}

function RootLicense_Cloud_CreateAccount(array $params)
{
...
// send to the rootlicense severe
    ...
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, 'http://apinew.rootlicense.com/api/v1/addcloud');
...
function RootLicense_Cloud_SuspendAccount(array $params)
{
    $result = mysql_query("SELECT `value` FROM `tbladdonmodules` WHERE `module` = 'rootlicense_addons' && `setting`='Email'");
    $email= mysql_fetch_array($result);
...
function RootLicense_Cloud_UnsuspendAccount(array $params)
{
    $result = mysql_query("SELECT `value` FROM `tbladdonmodules` WHERE `module` = 'rootlicense_addons' && `setting`='Email'");
    $email= mysql_fetch_array($result);
...
function RootLicense_Cloud_TerminateAccount(array $params)
{
    $result = mysql_query("SELECT `value` FROM `tbladdonmodules` WHERE `module` = 'rootlicense_addons' && `setting`='Email'");
    $email= mysql_fetch_array($result);
...
function RootLicense_Cloud_Renew(array $params)
{
    $result = mysql_query("SELECT `value` FROM `tbladdonmodules` WHERE `module` = 'rootlicense_addons' && `setting`='Email'");
    $email= mysql_fetch_array($result);
...
function RootLicense_Cloud_ClientArea($params){
...
// Attempt to decode response as json
        ...
        if ($jsonData["result"]=="success"){
            ...
            $code = "<a href='/cp/clientarea.php?action=productdetails&id=".$params["serviceid"]."'><span class='badge'>Your license IP has been changed successfully. please click here</span></a>";
            return $code;
        }
        else {
